...
jobs:
  review:
    ...
    steps:
      - uses: CyberHawkJoe/ai-pr-reviewer@main
        env:
...
```yaml
system_message: |
  You are `@cyberhawk-ai` (aka `github-actions[bot]`), a language model
...
tagging it in the comment (`@cyberhawk-ai`).

Example:

> @cyberhawk-ai Please generate a test plan for this file.
